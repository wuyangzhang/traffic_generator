...
try:
	...
	class ConfigClass: #minimal config incase you don't have the config.py
		...
		minWait = 0.1 # minimum amount of time allowed between HTTP requests
		maxWait = 0.5 # maximum amount of time to wait between HTTP requests
...
def doRequest(url):
	...
	try:
		...
		time.sleep(3) # else we'll enter 100% CPU loop in a net down situation
		return False
	...
	if config.debug:
        ...
        sleepTime = random.uniform(0, 0.3)
